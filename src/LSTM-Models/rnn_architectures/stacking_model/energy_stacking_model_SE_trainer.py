...
from tfrecords_handler.moving_window.tfrecord_mean_feature_reader import TFRecordReader
        ...
        with tf.Session() as session:
            ...
            while True:
                try:
                    ...
                    mean_value = validation_data_batch_value[3][array_first_dimension, last_indices, 0]

                    level_values = validation_data_batch_value[3][array_first_dimension, last_indices, 1]

                    last_validation_outputs = validation_output[array_first_dimension, last_indices]
                    converted_validation_output = np.exp(
                         level_values[:, np.newaxis] + last_validation_outputs)
		    converted_validation_output = converted_validation_output - 1
                    converted_validation_output = converted_validation_output*mean_value[:, np.newaxis]
                    ...
                    converted_actual_values = np.exp(
                        level_values[:, np.newaxis] + actual_values)
		    converted_actual_values = converted_actual_values -1
                    converted_actual_values = converted_actual_values * mean_value[:, np.newaxis]
