...
for (validation in c(TRUE, FALSE)) {
  for (idr in 1 : nrow(df_train)) {
    print(idr)
    OUTPUT_PATH = paste(OUTPUT_DIR, "energy_baseline", sep = '/')
    ...
    if (validation) {
      sav_df = matrix(NA, ncol = (5 + input_size + max_forecast_horizon), nrow = nrow(input_windows))
      ...
      sav_df[, (input_size + max_forecast_horizon + 4)] = time_series_mean
      sav_df[, (input_size + max_forecast_horizon + 5)] = meanvalues
    }else {
      sav_df = matrix(NA, ncol = (input_size + 2 + max_forecast_horizon), nrow = nrow(input_windows))
      sav_df = as.data.frame(sav_df) 
      sav_df[, 1] = paste(idr - 1, '|i', sep = '')
      sav_df[, 2 : (input_size + 1)] = input_windows
      sav_df[, (input_size + 2)] = '|o'
