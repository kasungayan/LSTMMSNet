...
for (validation in c(TRUE, FALSE)) {
  for (idr in 1 : nrow(df_train)) {
    print(idr)
    OUTPUT_PATH = paste(OUTPUT_DIR, "energy_mstl7", sep = '/')
    ...
    if (! validation) {
      ...
      # apply stl
    stl_result = tryCatch({
      sstl = mstl(msts(time_series_log, seasonal.periods = c(seasonality_period_1,seasonality_period_2, seasonality_period_3)), s.window = 7)
      seasonal_vect = as.numeric(sstl[, 3]) + as.numeric(sstl[, 4]) +  as.numeric(sstl[, 5])
