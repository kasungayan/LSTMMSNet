...
for (validation in c(TRUE, FALSE)) {
  for (idr in 1 : nrow(df_train)) {
    print(idr)
    OUTPUT_PATH = paste(OUTPUT_DIR, "energy_prophet", sep = '/')
    ...
    stl_result = tryCatch({
      ...
      seasonal_vect1 = as.numeric(sstl_comp$daily) 
      seasonal_vect2 = as.numeric(sstl_comp$weekly)
      seasonal_vect3 = as.numeric(sstl_comp$yearly)
